...
from models import dual_encoder, esim_attention, sentence_bilinear_attention


class ESIMUttAttGlove(object):
    ...
    def _inference(self, context, context_len, utterances, utterances_len,
                   context_sentence, context_sentence_len, tot_context_len, speaker):
        ...
        """sentence bilinear attention"""
        context_sentence_embedded = tf.nn.embedding_lookup(word_embeddings, context_sentence)
        sentence_bilatt = sentence_bilinear_attention.SentenceBilinearAtt(
        ...
        sentence_bilatt.pos_emb_bool = True
        sentence_bilatt.user_emb_bool = True
        sentence_attention_joined_feature = sentence_bilatt.get_sentence_joined_feature()

        joined_feature = tf.concat(axis=-1, values=[esim_word_joined_feature, sentence_attention_joined_feature])
    ...
    def build_train_graph(self):
        ...
        with tf.variable_scope("inference", reuse=False):
            self.logits = self._inference(self.context_ph, self.context_len_ph,
                                          ...
                                          self.context_sentence_ph, self.context_sentence_len_ph,
            ...
            with tf.device('/gpu:%d' % gpu_id):
                with tf.variable_scope("inference", reuse=tf.AUTO_REUSE):
                    logits = self._inference(context_ph[i], context_len_ph[i],
                                             ...
                                             context_sentence_ph[i], context_sentence_len_ph[i],
    ...
    def ubuntu_train(self, saved_file=None):
        def train(self, saved_file=None):
            ...
            with sess.as_default():
                ...
                for epoch_completed in range(self.hparams.num_epochs):
                    ...
                    data_process = DataProcess(self.input_path, "train", self.word2id)
    ...
    def run_evaluate(self, sess, type, data_path, test_case=1):
        ...
        self._logger.info("MRR: %.f" % avg_mrr)
